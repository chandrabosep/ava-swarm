...
async function main() {
  const row = await db.agentState.findUnique({
    where: { agent_safeAddress: { agent: 'pm', safeAddress: SAFE } },
  });
  if (!row) {
    console.log(`No PM state for ${SAFE} — next tick will be the first.`);
    return;
  }
  ...
  console.log(`✓ Cleared PM lastTick for ${SAFE}. Next poll (≤60s) will fire.`);
  await db.$disconnect();
}
